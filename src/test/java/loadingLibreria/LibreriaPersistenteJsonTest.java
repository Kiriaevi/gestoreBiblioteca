...
public class LibreriaPersistenteJsonTest {
    ...
    @ParameterizedTest
    @ValueSource(strings = {"test", "fileCheSicuramenteNonEsiste"})
    void testFileAlreadyExistsAndIfNotThenItsCreated(String filepath) throws IOException {
        testFile = File.createTempFile(filepath, ".json");
        LibreriaPersistenteAbstract lib = new LibreriaPersistenteJSON(testFile.getAbsolutePath());
        boolean result = lib.onInit();
        assertTrue(result);
